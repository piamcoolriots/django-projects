...
def store(request, q):
    ...
    if len(q) > 2:
        ...
        term.tracking_id = statistics.tracking_id(request)
        term.user = None
...
def _prepare_words(search_text):
    ...
    for common in STRIP_WORDS: # get each common word from STRIP_WORDS
        if common in words: # look for a match in words
            words.remove(common) # stripe it out
    return words[0:5]